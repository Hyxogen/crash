...
template<typename Type>
bool SimpleAssertEqual(const SimpleTest& test, Type expected, Type actual, std::string file, int line, std::string message);
template<typename Type>
bool SimpleAssertArrayEqual(const SimpleTest& test, Type *array1, Type *array2, int count, std::string file, int line, std::string message);
...
template<typename Type>
bool SimpleAssertArrayEqual(const SimpleTest& test, const Type *array1, const Type *array2, int count, std::string file, int line, std::string message) {
	(void)message;
	for (int i = 0; i < count; i++) {
		if (!SimpleAssertEqual<Type>(test, array1[i], array2[i], file, line,
			"at: " + std::to_string(i)))
			return false;
	}
	return true;
...
/*Make assert stop running other tests*/
#define _SIMPLE_TEST_FAIL(file, line, message)											SimpleDoFail(*this, #file ":" #line " - " #message)

#define _SIMPLE_ASSERT(condition, file, line, message)									{ if (condition) {} else { _SIMPLE_TEST_FAIL(file, line, message); return; }}

#define _SIMPLE_ASSERT_EQUAL(expected, actual, file, line, message)						{ if (SimpleAssertEqual(*this, expected, actual, file, line, message)) {} else { return; } }
